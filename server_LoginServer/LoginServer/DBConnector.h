...
public:
	...
	{
		// 초반 활성화 시간은 제외하기위해서
		REGISTER_SLOW_QUERY_TIMING = 200,
		// 쿼리 widechar기준 최대 문자 수
		QUERY_SIZE = 2048,
		// n회마다 느린 쿼리 목록을 로그로 남기기
		TIMING_WHEN_WRITE_SLOW_SET = 1000000,
	...
	CMySqlConnector(const char* hostIP = nullptr,
		const char* id = nullptr, const char* pw = nullptr,
		const char* schema = nullptr, unsigned short port = 0
		) : _host{},
		_conn{}, _sql_result(nullptr), _id(nullptr),
		_pw(nullptr), _schema(nullptr),
		_port(port), _connectCnt(0),
		_curAQuery{}, _curWQuery{}, _querySendCnt(0)
	{
		if(hostIP != nullptr)
...
private:
	CMySqlConnector(const CMySqlConnector&) = delete;
	CMySqlConnector& operator=(const CMySqlConnector&) = delete;
...
public:
	CDBWriter(c_syslog* plog)
	{
		...
		if (_hEnqueueEvent == NULL)
			__debugbreak();
		_hExit = CreateEventW(NULL, TRUE, FALSE, NULL);
		if (_hExit == NULL)
			__debugbreak();

		std::thread t(&CDBWriter::DBWriterProc, this);
		_thread = std::move(t);
	}
	~CDBWriter()
	{
	...
	void Quit();
	void RequestCreateWeapon();
	void RequestGetWeaponInfo();
	void RequestUpdateWeapon();

	bool isDanger() { return (_queue.GetSize() > 500); }
	int GetQueueSize() { return _queue.GetSize(); }

	double GetDBAvg() { return dbTotalTime / cnt; }
	double GetDBMax() { return dbMaxTime; }
	double GetTotalAvg() { return totalTime / cnt; }
	double GetTotalMax() { return maxTime; }
private:
	CMySqlConnector _conn;
	std::thread _thread;
	HANDLE _hEnqueueEvent;
	HANDLE _hExit;
	CLockFreeQueue<CPacket*> _queue;
	c_syslog* _plog;
...
public:
	...
	uint64_t befDeqCnt = 0;
	uint64_t curDeqCnt = 0;
	uint64_t DeqTps = 0;
...
private:
	...
	void RequestCreateWeapon(CPacket* pMessage);
	void RequestGetWeaponInfo(CPacket* pMessage);
	void RequestUpdateWeapon(CPacket* pMessage);
};
*/
