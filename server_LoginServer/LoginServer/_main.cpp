...
int main()
{
	...
	CLoginServer::stLoginServerOpt& opt = g_server.GetOption();
	...
	int tick;
	const Net::CServer::ServerMonitor& pSMonitor = g_server.GetServerMonitor();
	const Net::CClient::ClientMonitor& pCmonitor = g_client.GetClientMonitor();
	wchar_t buffer[5000];
	int storeMonitor = 0;

	int threadNum = g_server.GetOption().iWorkerThreadCreateCnt;

	tm starttm;
	time_t starttime = pSMonitor->GetServerStartTime();
	localtime_s(&starttm, &starttime);
	std::wstring_view processName = Net::CNetProcess::GetProcess_Name();
	Net::CNetProcess::OffSystemMonitor();
...
#pragma region CONSOLE_WRITE
		...
		swprintf_s(buffer + wcslen(buffer), 5000 - wcslen(buffer),
			L"|       UserCnt : %5d / %5d |  Total Login: %llu \n", g_userManager.GetUserCnt(), opt.iMaxUsers, g_server.GetLoginMonitor()->GetTotalLoginCount());
		swprintf_s(buffer + wcslen(buffer), 5000 - wcslen(buffer),
			L"| UserStructCnt : %5d         | Account Cnt: %d \n", g_userManager.GetUserStructCnt(), g_userManager.GetAccountCnt());
		swprintf_s(buffer + wcslen(buffer), 5000 - wcslen(buffer),
			L"|     Duplicate : %lld \n", g_server.GetLoginMonitor()->GetDuplicateDisconnectCnt());
		swprintf_s(buffer + wcslen(buffer), 5000 - wcslen(buffer),
			L"============================================================================================================ \n");
		swprintf_s(buffer + wcslen(buffer), 5000 - wcslen(buffer),
		...
		swprintf_s(buffer + wcslen(buffer), 5000 - wcslen(buffer),
			L"|                                                 |      Login Tps : %5d /sec \n", g_server.GetLoginMonitor()->GetLoginSuccessTps());
		swprintf_s(buffer + wcslen(buffer), 5000 - wcslen(buffer),
			L"============================================================================================================ \n");
		swprintf_s(buffer + wcslen(buffer), 5000 - wcslen(buffer),
