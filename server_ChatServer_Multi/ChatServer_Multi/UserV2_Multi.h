...
private:
	CUser():_sessionId(0), _state(STATE_NONE), _lastRecvTime(0), _lock(),
		_lastHeartBeatTime(0), _lastLoginTime(0), _lastSectorMoveTime(0), _lastChatTime(0),
		_strangeCnt(0), _accountNo(-1), _curPos{-1, -1}, _id{}, _nickName{}, _sessionKey{}
	{
	...
	DWORD _lastLoginTime;	// 로그인 시점
	DWORD _lastSectorMoveTime;
	DWORD _lastChatTime;
	...
	stSectorPos _curPos;
	wchar_t _id[ID_SIZE];
	wchar_t _nickName[NICK_SIZE];
	char _sessionKey[KEY_SIZE];
};
//-----------------------------------------------------
...
public:
	...
	CUser* CreateWaitLogin(uint64_t sessionId, in_addr ip, wchar_t* wip, DWORD time);
	...
	void ReleaseUser(CUser* pUser)
	{
		...
		if (pUser->_state != CUser::STATE_NONE)
		{
				...
				&& pUser->_state != CUser::STATE_WAIT_IDENTIFY)
			{
				...
				// 유저라서 정리 필요 (필드 내일 경우)
				//------------------------------------
				if (pUser->_state == CUser::STATE_INFIELD)
				{
					int ix = pUser->_curPos.iX;
					int iy = pUser->_curPos.iY;
					g_field.RemoveSector(iy, ix, pUser->_sessionId);
				}
				//------------------------------------
				// (로그인 정리)
				//------------------------------------
